import { UNIT_LABELS, formatLength, roundToUnit, toInches } from '../lib/units'
...
export function DeskSettings({ preferences, onChange }: Props) {
  const { unit, deskEnabled, deskWidth, deskDepth, deskX, deskY } = preferences

  return (
    <section className="mb-6">
      <h2 className="mb-2.5 text-base font-semibold text-[var(--text-secondary)]">Desk</h2>
      <div className="flex flex-col gap-4 rounded-[10px] border border-[var(--border)] bg-[var(--surface-1)] p-3">
        <label className="flex cursor-pointer items-center gap-2.5">
          <input
            ...
            className="size-4"
          />
          <span className="text-sm text-[var(--text-secondary)]">Show my desk on top view</span>
        </label>
